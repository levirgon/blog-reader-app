...
public class MyOwnPostFragment extends Fragment implements PaginationAdapterCallback {
    ...
    private void updateUI() {
        if (mAdapter == null) {
            mAdapter = new BlogRecycleAdapter(getActivity().getApplicationContext(), (ListFragment.OnItemSelectedInterface) mListener, mCallback);
        ...
        callResponse().enqueue(new Callback<AllpostsResponse>() {
            ...
            public void onResponse(Call<AllpostsResponse> call, Response<AllpostsResponse> response) {
                ...
                if (response.isSuccessful()) {
                    ...
                    Toast.makeText(getContext(),results.toArray().toString(),Toast.LENGTH_SHORT).show();
    ...
    private Call<AllpostsResponse> callResponse() {
     ...
     int userId =    SharedPrefManager.getInstance(getContext()).getUserId();
        return mService.getOwnPostsByPage(userId,token,mCurrentPage);
    }
